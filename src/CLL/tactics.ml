...
module Clltactics =
  ...
  struct
    ...
    let WITH_TAC : actiontactic =
      ...
      let TAG_MERGED_TAC lbl originputs tag st (asl,_ as gl) = 
        try (
          ...
          (* We expect FILTER_TAC or withTac to have been used, so only 1 new input has been created *)
          if (length merged = 1) 
      ...
      (* Intermediate channels produced by FILTER_TAC get renamed when matched in the WITH rules *)
      (* We ultimately use the metavariable instantiations generated by the whole process to 
         instantiated the merged channels we recorded. *)
      let meta_inst_merge ((_,inst),_,_ as gs:goalstate),st = gs,Actionstate.inst_merge inst st in

      (* We compute common channels so we can report them as merged. 
         Channels that are not common are sent to matchInputsTac. *)

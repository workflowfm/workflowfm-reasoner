...
module Piviz_make (Composer : Composer_type) =
  struct
    ...
    let deploy_string: Process.t -> Process.t list -> string =
      fun p deps ->
    ...
    let deploy: Process.t -> Process.t list -> Composer.Response.t =
      fun p deps ->
      Deploy ("PiViz",["",deploy_string p deps,true])
